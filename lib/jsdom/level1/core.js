...
const validateName = require('../living/helpers/validate-names').name;
...
const NODE_INDEX = require('../living/helpers/shared-symbols').nodeIndex;

// utility functions
const attachId = function(id,elm,doc) {
  if (id && elm && doc) {
    if (!doc._ids[id]) {
      doc._ids[id] = [];
    }
...
const detachId = function(id,elm,doc) {
  var elms, i;
  if (id && elm && doc) {
...
core.Node.prototype = {
  ...
  _indexOf: function(/*Node*/ child) {
    const nodeIndex = child[NODE_INDEX];
    if (nodeIndex !== null && child._parentNode === this) {
      return nodeIndex;
    }
  ...
  insertBefore :  function(/* Node */ newChild, /* Node*/ refChild) {
    ...
    if (newChild.nodeType === DOCUMENT_FRAGMENT_NODE && !newChild._templateContent) {
      let tmpNode, i = newChild._childNodes.length;
      while (i-- > 0) {
      ...
      if (refChild == null) {
        ...
        newChild[NODE_INDEX] = this._childNodes.length - 1;
      } else {
        let refChildIndex = this._indexOf(refChild);
        if (refChildIndex == -1) {
          throw new core.DOMException(core.DOMException.NOT_FOUND_ERR);
        }
        this._childNodes.splice(refChildIndex, 0, newChild);
        for (let i = refChildIndex; i < this._childNodes.length; ++i) {
          this._childNodes[i][NODE_INDEX] = i;
        }
      }
  ...
  removeChild : function(/* Node */ oldChild){
    ...
    for (let i = oldChildIndex; i < this._childNodes.length; ++i) {
      this._childNodes[i][NODE_INDEX] = i;
    }

    oldChild._parentNode = null;
    oldChild[NODE_INDEX] = null;
    this._modified();
    oldChild._detach();
...
const tagRegEx = /[^\w:\d_\.-]+/i;
const entRegEx = /[^\w\d_\-&;]+/;
const invalidAttrRegEx = /[\s"'>/=\u0000-\u001A]/;
...
inheritFrom(core.Node, core.Attr, {
  ...
  set nodeValue(value) {
    ...
    // going backwards is more efficient because it avoids splice()
